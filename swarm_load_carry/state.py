...
class CS_type(Enum):
    ...
    LLA=2,         # Planetary co-ordinate system: Latitude, Longitude, Altitude. Frame can only be 'world'
    ENU=3,                  # Local tangent plane body CS: East, North, Up (ROS2 default) 
    NED=4                   # Local tangent plane body CS: North, East, Down (Pixhawk default)
...
class State():
    def __init__(self, frame, cs_type, pos=np.array([0.0, 0.0, 0.0]), att=qt.array([0.0, 0.0, 0.0, 1.0]), vel=np.array([0.0, 0.0, 0.0])):
        self.frame = frame
